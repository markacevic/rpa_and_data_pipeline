...
def test_process_market_data(raw_data_file):
    ...
    assert df.loc[0, 'category'] == 'ЧАЕВИ,КАКАО,КОРНФЛЕКС - ВИТ.ХРАНА-КОРНФЛЕКС,ЖИТАРИЦИ'
    ...
    for col, expected_dtype in expected_schema.items():
        assert df[col].dtype == expected_dtype, f"Column '{col}' has dtype {df[col].dtype}, expected {expected_dtype}"