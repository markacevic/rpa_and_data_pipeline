...
def main():
    ...
    parser.add_argument(
        "market",
        type=str,
        help="The name of the market to scrape (e.g., 'Vero', 'Tinex').",
    )
    parser.add_argument(
        "--browser",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="The maximum number of products to scrape per page.",
    )
    parser.add_argument(
        "--total-limit",
        type=int,
        default=None,
        help="The maximum total number of products to scrape across all pages.",
    )
    parser.add_argument(
        "--no-headless",
        action="store_true",
    ...
    if not config:
        logging.error(
            f"Configuration for market '{args.market}' not found in settings.py."
        )
        return

    setup_logging()
    logging.info(
        f"Starting scraper for '{args.market}' with a limit of {args.total_limit} products."
    )
    ...
    try:
        ...
        if not output_files:
            logging.warning(
                f"No data was scraped for {args.market}. Skipping processing."
            )
            return
        ...
        if not all_processed_data:
            logging.warning(
                f"No processable product data found for {args.market}. No report generated."
            )
            return
        ...
        if validated_df.empty:
            logging.warning(
                "No data remained after validation. No reports will be generated."
            )
            return

        # --- Generate Analytics Report Step ---
        logging.info("Generating summary analytics report...")
        analytics_report_path = os.path.join(
            "outputs", "reports", f"{args.market.lower()}_summary_analytics_report.json"
        )
        ...
        output_csv_path = os.path.join(
            output_dir, f"{args.market.lower()}_processed_data.csv"
        )

        validated_df.to_csv(output_csv_path, index=False, encoding="utf-8")
        logging.info(
            f"Successfully processed and saved {len(validated_df)} validated records to {output_csv_path}"
        )
