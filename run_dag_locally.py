...
def run_pipeline_locally(
    ...
    try:
        # --- 1. Scrape Data ---
        logging.info(
            f"--- Starting local scrape task for '{market_name}' (browser visible: {not headless}) ---"
        )
        scrape_data_task(
        ...
        logging.info(
            f"--- Local pipeline run for '{market_name}' finished successfully! ---"
        )

    except Exception as e:
        logging.error(
            f"An error occurred during the local run for '{market_name}': {e}",
            exc_info=True,
        )
