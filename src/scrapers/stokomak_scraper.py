...
if __name__ == '__main__':
    ...
    logging.info(f"--- Starting direct test run for StokomakScraper (limit: {TEST_PRODUCT_LIMIT} products) ---")
    ...
    try:
        with StokomakScraper(
            ...
            if output_files:
                logging.info(f"StokomakScraper direct test run finished successfully. Data saved to: {output_files}")
            else:
                logging.warning("StokomakScraper test run completed, but no data was saved.")
            
    except Exception as e:
        logging.error(f"An error occurred during the StokomakScraper test run: {e}", exc_info=True)