...
class DataProcessor(ABC):
    ...
    UNIT_TYPE_MAP = { # This is a mapping of the unit types to the unit column in the output
        'volume': 'l',
        'weight': 'kg',
        'pieces': 'piece'
    }
    ...
    MEASUREMENT_PATTERNS = { # This is a mapping of the measurement patterns to the standardized units
        'volume': {
            ...
            'multipliers': {
                'МЛ': 1, 'Л': 1000, 'ЛТ': 1000, 'ЛИТАР': 1000, 'ЛИТРИ': 1000,
                'ML': 1, 'L': 1000, 'LT': 1000
            }
        },
        'weight': {
            'units': ['Г', 'ГР', 'КГ', 'ГРАМ', 'КИЛОГРАМ', 'ГРАМОВИ', 'КИЛОГРАМИ', 'КГР', 'G', 'GR', 'KG'],
            'pattern': r'(\d+(?:\.\d+)?)\s*(Г|ГР|КГ|ГРАМ|КИЛОГРАМ|ГРАМОВИ|КИЛОГРАМИ|КГР|G|GR|KG)',
            'multipliers': {
                'Г': 1, 'ГР': 1, 'КГ': 1000, 'ГРАМ': 1, 'КИЛОГРАМ': 1000,
                'ГРАМОВИ': 1, 'КИЛОГРАМИ': 1000, 'КГР': 1000,
                'G': 1, 'GR': 1, 'KG': 1000
            }
        },
        'pieces': {
            'units': ['КОМ', 'ПАР', 'БРОЈ', 'ПАРЧЕ', 'PAR', 'PARCE', 'PARCHE', 'PCS', 'PC'],
            'pattern': r'(\d+)\s*(КОМ|ПАР|БРОЈ|ПАРЧЕ|PAR|PARCE|PARCHE|PCS|PC)',
            'multipliers': {
                'КОМ': 1, 'ПАР': 1, 'БРОЈ': 1, 'ПАРЧЕ': 1,
                'PAR': 1, 'PARCE': 1, 'PARCHE': 1, 'PCS': 1, 'PC': 1
            }
        }
    }
    ...
    def create_product_data(self,
        ...
        ppu_data = self._extract_quantity_and_unit_from_price_per_unit(price_per_unit, current_price)
        ...
        # Calculate discount
        discount = 0.0
        if regular_price_val and current_price_val and regular_price_val > 0 and current_price_val < regular_price_val:
            discount = round(((regular_price_val - current_price_val) / regular_price_val) * 100, 2)
    ...
    def _extract_quantity_and_unit_from_product_name(self, product_name: str) -> Dict[str, Any]:
        ...
        for measurement_type, config in self.MEASUREMENT_PATTERNS.items():
            ...
            if match:
                ...
                standard_quantity = self._convert_to_standard(quantity, unit, measurement_type)
                return {
                    'quantity': quantity,
                    'unit': unit,
    ...
    def _extract_quantity_and_unit_from_price_per_unit(self, price_per_unit: str, current_price: str) -> Dict[str, Any]:
        ...
        if current_price_clean and ppu_price and abs(current_price_clean - ppu_price) < 0.01:
            return {'quantity': None, 'unit': None, 'unit_type': None}
    ...
    def _extract_price_per_unit_value(self, ppu_str: str) -> Optional[float]:
      ...
      # and standardize it, then use current price to calculate price per unit (standardized)
        ...
        for measurement_type, config in self.MEASUREMENT_PATTERNS.items():
            match = re.search(r'(\d+(?:\.\d+)?)\s*ДЕН\s*/\s*(\w+)', ppu_str.upper(), re.IGNORECASE)
        ...
        for pattern in patterns:
            ...
            if match:
                try:
                    return float(match.group(2)) if len(match.groups()) == 2 else float(match.group(1))
                except ValueError:
                    continue
        return None
    ...
    def _convert_to_standard(self, quantity: float, unit: str, unit_type: str) -> float:
        ...
        return base_quantity / 1000.0 if unit_type in ['volume', 'weight'] else base_quantity
    ...
    def _extract_availability(self, availability: str) -> Optional[bool]:
        ...
        available_indicators = ['DA', 'YES', 'TRUE', '1', 'AVAILABLE', 'НА РАСПОЛАГАЊE', 'ДА']
